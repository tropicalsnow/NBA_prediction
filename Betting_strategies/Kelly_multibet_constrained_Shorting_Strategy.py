def Kelly_multibet_constrained_Shorting_Strategy(our_prediction, market_odds, Probs, alpha, beta,  max_exposure, max_bet_pcnt, balance, existingStakes=None):
    ...
    for i in range(len(Probs)):
        if all(market_odds[i]) > 0:
            ...
            if s1<0 and s2<0:
                #No bet
                pass
            elif (s1>0 and s2<0) or (s1>0 and p1> np.log(b1*(1+b2)/(1+b1)) / np.log(b1*b2)  ):
                #Bet on side A 
                Bets[i]=1
            else:
                #Bet on side B
    ...
    constraints = [cvx.sum(b_rck) == 1, b_rck >= 0.0, cvx.max(b_rck@I)<=max_exposure, risk_constraint, b_rck[0:n]<=max_bet_pcnt] 
    ...
    constraints = [cvx.sum(b_qrck) == 1, b_qrck >= 0,  cvx.max(b_qrck@I)<=max_exposure, risk_constraint_qrck,  b_qrck[0:n]<=max_bet_pcnt] 
    #include existing stake constraints
    for i in range(len(existingStakes)):
        if not math.isnan(existingStakes[i]):
            constraints.append(b_qrck[i]==existingStakes[i]) #also include 0 bets as constraints. Only nans are unconstrained
